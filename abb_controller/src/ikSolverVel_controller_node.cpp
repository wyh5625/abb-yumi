...
void cartVelCallback(const std_msgs::Float64MultiArray &msg) {
    ...
    for (int i = 0; i < msg.data.size(); i += 3) {
        ...
        std::cout << eeLinks[i] << " v_x:" << msg.data[i] << " v_y:" << msg.data[i + 1] << " v_z:" << msg.data[i + 2]
                  << " m/s ";
    }
...
int main(int argc, char **argv) {
    ...
    ros::Publisher cart_vPublisher = node.advertise<std_msgs::Float64MultiArray>("/yumi/ikSloverVel_controller/state",
                                                                                 1);
    ros::Publisher cart_pPublisher = node.advertise<std_msgs::Float64MultiArray>(
            "/yumi/ikSloverVel_controller/ee_cart_position", 1);
    ros::Publisher cart_pVisPublisher = node.advertise<geometry_msgs::PoseArray>(
            "/yumi/ikSloverVel_controller/ee_cart_position_vis", 1);

    ros::Publisher jointGroupVelPublisher = node.advertise<std_msgs::Float64MultiArray>(
            "/yumi/joint_group_vel_controller/command", 1);
    ...
    while (ros::ok()) {
        ...
        if (ret_l0 == SolverI::E_NOERROR && ret_r0 == SolverI::E_NOERROR) {
            ...
            for (int i = 0; i < JOINT_NUMBER; i++) {
                ...
                // chain method
                if (i < JOINT_NUMBER / 2)
                    joint_velocity = qdot_out_l(i);
                else
                    joint_velocity = qdot_out_r(i - 7);
