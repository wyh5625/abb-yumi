...
{
    ROS_INFO_STREAM("Creating a abb_yumi communication interface for: " << name <<" with "<<n_joints_<<" joints");
	...
	double *const lower_limit, double *const upper_limit)
{
    ...
    *upper_limit = std::numeric_limits<double>::max();
    ...
    if (urdf_model != NULL)
    {
	std::shared_ptr<const urdf::Joint> urdf_joint = urdf_model->getJoint(joint_name);
	std::shared_ptr<const urdf::Joint> urdf_joint_sitffness = urdf_model->getJoint(joint_name + std::string("_stiffness"));
	if (urdf_joint != NULL)
	{
